...
ensure_user() {
    ...
    if id cosmic-greeter >/dev/null 2>&1; then
        say "groups: cosmic-greeter (greetd compositor user)"
        for g in seat video input audio; do
            if getent group "$g" >/dev/null 2>&1; then
                if ! id -nG cosmic-greeter | tr ' ' '\n' | grep -qx "$g"; then
                    addgroup cosmic-greeter "$g" 2>/dev/null \
                        && ok "added cosmic-greeter to $g" \
                        || warn "could not add cosmic-greeter to $g"
                else
                    ok "cosmic-greeter already in $g"
                fi
            fi
        done
    fi
