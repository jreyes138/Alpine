...
install_cosmic_packages() {
    ...
    # Bare-metal GPU firmware.  QEMU VMs have emulated GPUs that don't
    # need firmware blobs; real hardware does.  --bare-metal adds
    # linux-firmware so AMD/NVIDIA/Intel devices have the right blobs.
    if [ "$DO_BARE_METAL" = "1" ]; then
        say "bare-metal GPU firmware"
        apk add $APK_FLAGS linux-firmware linux-firmware-amd linux-firmware-intel \
            || warn "linux-firmware install failed - GPU may not work"
    fi
