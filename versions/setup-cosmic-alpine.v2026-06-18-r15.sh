...
#   7. Use existing setup-alpine user (or create "cosmic" if none), add to
#      desktop groups (audio input video netdev wheel seat)
#   8. Install COSMIC packages (cosmic-session meta + cosmic* apps)
#   9. Install terminal (wezterm), file manager, screenshot, common utilities
#      (fastfetch, btop, bat, eza, micro, git, wget, curl) and nerd fonts
#      (FiraCode, JetBrains Mono).  No browser by default — use --browser
#      or install Brave/Tutanota via the flatpak default apps.
...
#
# Options:
#   -u, --user NAME         Non-root user to use (default: $SUDO_USER, or the first
#                           non-root account created by setup-alpine, or "cosmic")
#   -U, --no-user           Skip user creation/upgrade (use on a system that
#                           already has a desktop user)
#   -b, --browser PKG       Optional browser package (default: none; pass "chromium" etc. to install)
...
# ---------- defaults ----------
# Pick the target user in this order:
#   1. --user NAME (explicit override)
#   2. $SUDO_USER (the operator who invoked sudo/doas)
#   3. The first non-root, non-system user with a real shell (typically
#      the account created by setup-alpine)
#   4. "cosmic" (last resort: create a new user)
# This way, on a fresh Alpine install where setup-alpine already created
# "joser" (or whatever), the script reuses that user instead of making
# a second one called "cosmic".
detect_existing_user() {
    # First non-root account with a real login shell and a home dir.
    # getent passwd: uid >= 1000, uid < 65534, shell that isn't /sbin/nologin
    getent passwd \
        | awk -F: '$3 >= 1000 && $3 < 65534 && $7 !~ /(nologin|false|halt|shutdown|sync)$/ {print $1}' \
        | head -1
}
if [ -n "${SUDO_USER:-}" ] && [ "$SUDO_USER" != "root" ]; then
    TARGET_USER="$SUDO_USER"
else
    EXISTING=$(detect_existing_user)
    if [ -n "$EXISTING" ]; then
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -S|--no-audio)        DO_AUDIO=0; shift ;;
        -W|--no-power)        DO_POWER=0; shift ;;
        -Y|--no-systemd-shim) DO_SYSTEMD_SHIM=0; shift ;;
        ...
        -P|--default-password) DO_DEFAULT_PASSWORD=1; shift ;;
        --password)           DEFAULT_PASSWORD="$2"; DO_DEFAULT_PASSWORD=1; shift 2 ;;
        -h|--help)            usage; exit 0 ;;
...
ensure_user() {
    ...
    if id "$TARGET_USER" >/dev/null 2>&1; then
        ...
        # BusyBox adduser (Alpine) has no -p flag for non-interactive passwords.
        # Default: create with no password and prompt the operator.
        # --default-password: set a default password and force change on first
        #   login (chpasswd + passwd -e).  For VMs / automation.
        # --password PASS: same but with a custom password.
        adduser -D -g "COSMIC User" "$TARGET_USER" 2>/dev/null \
            || die "adduser failed (is '$TARGET_USER' a reserved name?)" 3
        if [ "$DO_DEFAULT_PASSWORD" = "1" ]; then
            # chpasswd reads "user:pass" pairs from stdin.  BusyBox chpasswd
            # is in the shadow package (already installed via polkit's deps).
            printf '%s:%s\n' "$TARGET_USER" "$DEFAULT_PASSWORD" \
                | chpasswd 2>/dev/null \
                || die "chpasswd failed - cannot set default password" 3
            # Force password change on first login.
            passwd -e "$TARGET_USER" 2>/dev/null \
                && ok "user $TARGET_USER created with default password (will be required to change on first login)" \
                || warn "user $TARGET_USER created with default password, but passwd -e failed"
        elif [ "$ASSUME_YES" = "1" ]; then
            ok "user $TARGET_USER created (no password - set later with: passwd $TARGET_USER)"
        else
...
install_optional_apps() {
    # Optional browser (none by default)
    if [ -n "$BROWSER_PKG" ]; then
        ...
        say "skipping native browser (use --browser or flatpak)"
    fi

    # Terminal emulator (wezterm)
    say "terminal: wezterm"
    apk add $APK_FLAGS wezterm 2>/dev/null \
        && ok "wezterm installed" \
        || warn "wezterm install failed (continuing)"

    # CLI utilities
    say "CLI utilities: fastfetch, btop, bat, eza, micro, git, wget, curl, htop, nano, sudo"
    apk add $APK_FLAGS \
        fastfetch btop bat eza micro git wget curl htop nano sudo \
        2>/dev/null \
        && ok "CLI utilities installed" \
        || warn "some CLI utilities failed (continuing)"

    # Nerd fonts (FiraCode + JetBrains Mono, both with Nerd Font patches)
    say "nerd fonts: FiraCode + JetBrains Mono"
    apk add $APK_FLAGS \
        font-fira-code-nerd font-jetbrains-mono-nerd \
        2>/dev/null \
        && ok "nerd fonts installed" \
        || warn "some nerd fonts failed (continuing — check if non-free repo is enabled)"
}
...
# Install and configure the audio stack (pipewire + wireplumber + ALSA/Pulse
# compatibility layers).  Without pipewire, cosmic-osd and cosmic-settings-daemon
# can spin at 100% CPU because they poll a non-existent audio service
# (see pop-os/cosmic-osd issues #70 and #162).  Alpine uses OpenRC, not
# systemd, so we wire pipewire up via two paths:
#   1. System-wide service: pipewire is the audio daemon for the system bus
#   2. Per-user autostart:   wireplumber + pipewire-pulse for the user session
#      (drop .desktop files in ~/.config/autostart/ so cosmic-session picks
#      them up via the XDG autostart spec).
install_audio_stack() {
    [ "$DO_AUDIO" = "1" ] || { say "skipping audio stack (--no-audio)"; return; }
    say "audio stack (pipewire + wireplumber)"

    # Install the package set
    if apk info -e pipewire wireplumber >/dev/null 2>&1; then
        ok "pipewire + wireplumber already installed"
    else
        apk add $APK_FLAGS \
            pipewire pipewire-pulse pipewire-alsa wireplumber \
            || die "pipewire/wireplumber install failed" 4
    fi

    # System-wide pipewire service (background audio for the system)
    if rc-service pipewire status 2>/dev/null | grep -q started; then
        ok "pipewire already running"
    else
        rc-update add pipewire default 2>&1 || true
        rc-service pipewire start 2>&1 \
            && ok "pipewire started" \
            || warn "pipewire did not start (will run per-user via autostart)"
    fi

    # User-session autostart: cosmic-session honors XDG autostart, so we
    # drop .desktop files for pipewire + wireplumber in /etc/xdg/autostart/
    # (system-wide) and they get inherited by every user.
    say "user-session autostart (pipewire + wireplumber .desktop files)"
...
Name=PipeWire
Exec=pipewire
Comment=Start the PipeWire audio server in the user session
Terminal=false
Categories=Audio;System;
X-GNOME-Autostart-Phase=PreDisplayServer
EOF
        ok "installed /etc/xdg/autostart/pipewire.desktop"
    else
        ok "/etc/xdg/autostart/pipewire.desktop already present"
    fi
...
Name=WirePlumber
Exec=wireplumber
Comment=Start the WirePlumber session manager for PipeWire
Terminal=false
Categories=Audio;System;
X-GNOME-Autostart-Phase=PreDisplayServer
EOF
        ok "installed /etc/xdg/autostart/wireplumber.desktop"
    else
        ok "/etc/xdg/autostart/wireplumber.desktop already present"
    fi

    # Disable pulseaudio if it's around (pipewire-pulse replaces it)
    if rc-service pulseaudio status 2>/dev/null | grep -q started; then
        warn "pulseaudio is running and will conflict with pipewire-pulse"
        warn "to stop: rc-service pulseaudio stop && rc-update del pulseaudio"
    fi
}

# Install and configure the power stack (upower + tuned-ppd for power-profiles).
# Without these, the COSMIC power applet shows "Power mode: backend not found"
# (no PowerProfiles DBus), the battery applet may not show battery level
# (no UPower), and the daemon polls can spin up CPU on laptops.  Alpine's
# upower package doesn't ship an OpenRC init script, so we write one.
install_power_stack() {
    [ "$DO_POWER" = "1" ] || { say "skipping power stack (--no-power)"; return; }
    say "power stack (upower + tuned-ppd for power-profiles-daemon)"

    # Install packages
    if apk info -e upower >/dev/null 2>&1; then
        ok "upower already installed"
    else
        apk add $APK_FLAGS upower || die "upower install failed" 4
    fi
    if apk info -e tuned-ppd tuned-ppd-openrc >/dev/null 2>&1; then
        ok "tuned-ppd + tuned-ppd-openrc already installed"
    else
        apk add $APK_FLAGS tuned-ppd tuned-ppd-openrc || die "tuned-ppd install failed" 4
    fi

    # Write /etc/init.d/upower (Alpine's upower package doesn't ship one)
    if [ ! -f /etc/init.d/upower ]; then
        say "creating /etc/init.d/upower (Alpine doesn't ship one)"
        cat > /etc/init.d/upower <<'EOF'
#!/sbin/openrc-run
# OpenRC init script for upowerd (UPower daemon)
# Provides org.freedesktop.UPower on the system bus.
# Required by: cosmic-settings (battery applet, lid close actions, suspend),
# gnome-power-manager, xfce4-power-manager, etc.

description="UPower daemon - battery/AC status and power management"
supervisor=supervise-daemon
command=/usr/libexec/upowerd
command_args_foreground="--verbose"

depend() {
    need dbus
}
EOF
        chmod 0755 /etc/init.d/upower
        ok "/etc/init.d/upower created"
    else
        ok "/etc/init.d/upower already present"
    fi

    # Enable + start upower
    if rc-service upower status 2>/dev/null | grep -q started; then
        ok "upower already running"
    else
        rc-update add upower default 2>&1 || true
        rc-service upower start 2>&1 \
            && ok "upower started" \
            || warn "upower did not start (will retry on next boot)"
    fi

    # Enable + start tuned (provides the tuned daemon, which tuned-ppd needs)
    if rc-service tuned status 2>/dev/null | grep -q started; then
        ok "tuned already running"
    else
        rc-update add tuned default 2>&1 || true
        rc-service tuned start 2>&1 \
            && ok "tuned started" \
            || warn "tuned did not start (tuned-ppd needs it for power-profiles)"
    fi

    # Enable + start tuned-ppd (power-profiles-daemon DBus backend)
    if rc-service tuned-ppd status 2>/dev/null | grep -q started; then
        ok "tuned-ppd already running"
    else
        rc-update add tuned-ppd default 2>&1 || true
        rc-service tuned-ppd start 2>&1 \
            && ok "tuned-ppd started" \
            || warn "tuned-ppd did not start (power-profiles DBus will be missing)"
    fi

    # Verify the DBus interfaces are up
    say "verifying DBus interfaces"
    if command -v dbus-send >/dev/null 2>&1; then
        if dbus-send --system --print-reply --dest=org.freedesktop.UPower \
                /org/freedesktop/UPower org.freedesktop.UPower.EnumerateDevices 2>/dev/null \
                | grep -q "object path"; then
            ok "UPower DBus interface responding"
        else
            warn "UPower DBus interface NOT responding (applet may show no battery)"
        fi
        if dbus-send --system --print-reply --dest=net.hadess.PowerProfiles \
                /net/hadess/PowerProfiles net.hadess.PowerProfiles.GetProfile 2>/dev/null \
                | grep -q "string"; then
            ok "PowerProfiles DBus interface responding (Battery/Balanced/Performance)"
        else
            warn "PowerProfiles DBus interface NOT responding (power applet: backend not found)"
        fi
    fi
}

# Install a tiny DBus shim for the systemd-only services that COSMIC
# (and other freedesktop apps) expect to find on the system bus:
#   - org.freedesktop.locale1   (keyboard layout - provided by systemd-localed)
#   - org.freedesktop.timedate1 (timezone - provided by systemd-timedated)
# Without these, COSMIC daemons create proxy objects that immediately fail
# with "no such name" and may spin retrying.  The shim registers empty
# stub interfaces on the same well-known names so calls return quickly.
#
# On Fedora, these are real systemd services.  On Alpine, they don't
# exist (we use OpenRC, not systemd).  The shim is the smallest possible
# workaround that keeps COSMIC functional without pulling in systemd.
#
# Disable with --no-systemd-shim if you'd rather have COSMIC's error
# path go through and debug from there.
install_systemd_shim() {
    [ "$DO_SYSTEMD_SHIM" = "1" ] || { say "skipping systemd shim (--no-systemd-shim)"; return; }
    say "systemd service shim (locale1 + timedate1 stubs for COSMIC)"

    # Install the dependency: py3-dbus-next (pure-python, no glib)
    if apk info -e py3-dbus-next >/dev/null 2>&1; then
        ok "py3-dbus-next already installed"
    else
        apk add $APK_FLAGS py3-dbus-next || die "py3-dbus-next install failed" 4
    fi

    # Drop the shim script
    if [ ! -f /usr/local/bin/cosmic-systemd-shim.py ]; then
        cat > /usr/local/bin/cosmic-systemd-shim.py <<'PYEOF'
#!/usr/bin/env python3
"""
Systemd service shim for COSMIC on Alpine Linux.
Provides stub org.freedesktop.locale1 and org.freedesktop.timedate1
DBus interfaces that respond to introspection and property reads
with empty/default values.  Without this, COSMIC's cosmic-settings-daemon
and cosmic-osd can spin polling for these systemd-only services.
"""
import asyncio
import signal
import sys
...
<node>
  <interface name="org.freedesktop.DBus.Introspectable">
    <method name="Introspect"><arg name="out" type="s" direction="out"/></method>
  </interface>
...
class StubInterface(ServiceInterface):
    ...
    @method()
    def Get(self, interface: 's', property: 's') -> 'v':
        return ('s', '')
    @method()
    def GetAll(self, interface: 's') -> 'a{sv}':
        return {}
    @method()
    def Set(self, interface: 's', property: 's', value: 'v'):
        return None

async def main():
    bus = await MessageBus(bus_type=BusType.SYSTEM).connect()
    for name in ('org.freedesktop.locale1', 'org.freedesktop.timedate1'):
        reply = await bus.request_name(name)
        if reply != RequestNameReply.PRIMARY_OWNER:
            print(f'warning: could not own {name}: {reply}', file=sys.stderr)
    locale1 = StubInterface('org.freedesktop.locale1')
    timedate1 = StubInterface('org.freedesktop.timedate1')
    bus.export('/org/freedesktop/locale1', locale1)
    bus.export('/org/freedesktop/timedate1', timedate1)
    print('cosmic-systemd-shim: registered stubs for locale1 and timedate1', file=sys.stderr)
...
if __name__ == '__main__':
    try:
        ...
        pass
PYEOF
        chmod 0755 /usr/local/bin/cosmic-systemd-shim.py
        ok "installed /usr/local/bin/cosmic-systemd-shim.py"
    else
        ok "/usr/local/bin/cosmic-systemd-shim.py already present"
    fi

    # Drop the OpenRC init script
    if [ ! -f /etc/init.d/cosmic-systemd-shim ]; then
        cat > /etc/init.d/cosmic-systemd-shim <<'EOF'
#!/sbin/openrc-run
# OpenRC init for the COSMIC systemd service shim.
# Provides stub DBus interfaces for org.freedesktop.locale1 and
# org.freedesktop.timedate1 so COSMIC's daemons don't spin polling
# for systemd services that don't exist on Alpine.

description="COSMIC systemd service shim (locale1 + timedate1 stubs)"
supervisor=supervise-daemon
command=/usr/local/bin/cosmic-systemd-shim.py
...
EOF
        chmod 0755 /etc/init.d/cosmic-systemd-shim
        ok "installed /etc/init.d/cosmic-systemd-shim"
    else
        ok "/etc/init.d/cosmic-systemd-shim already present"
    fi

    # Drop the DBus system policy that allows root to own these names.
    # Without this, dbus-daemon refuses the request_name call with
    # "Connection is not allowed to own the service ... due to security
    # policies in the configuration file".
    if [ ! -f /etc/dbus-1/system.d/cosmic-systemd-shim.conf ]; then
        say "creating /etc/dbus-1/system.d/cosmic-systemd-shim.conf"
        install -d /etc/dbus-1/system.d
        cat > /etc/dbus-1/system.d/cosmic-systemd-shim.conf <<'EOF'
<?xml version="1.0"?>
<!DOCTYPE busconfig PUBLIC "-//freedesktop//DTD D-BUS Bus Configuration 1.0//EN"
 "http://www.freedesktop.org/standards/dbus/1.0/busconfig.dtd">
<busconfig>
  <!-- Allow the shim (running as root) to own these systemd service names -->
  <policy user="root">
  ...
  <!-- Allow everyone to call into the shim -->
...
EOF
        ok "installed /etc/dbus-1/system.d/cosmic-systemd-shim.conf"
    else
        ok "/etc/dbus-1/system.d/cosmic-systemd-shim.conf already present"
    fi

    # Enable + start
    if rc-service cosmic-systemd-shim status 2>/dev/null | grep -q started; then
        ok "cosmic-systemd-shim already running"
    else
        rc-update add cosmic-systemd-shim default 2>&1 || true
        rc-service cosmic-systemd-shim start 2>&1 \
            && ok "cosmic-systemd-shim started" \
            || warn "cosmic-systemd-shim did not start (COSMIC may still spin on locale1/timedate1)"
    fi

    # If we just (re)wrote the DBus policy, dbus-daemon must reload its
    # config to pick it up.  ReloadConfig on the system bus is a no-op for
    # already-loaded rules but is required for the new policy to take effect.
    # The .conf files in /etc/dbus-1/system.d/ are only read on startup.
    if command -v dbus-send >/dev/null 2>&1; then
        dbus-send --system --print-reply --dest=org.freedesktop.DBus \
            / org.freedesktop.DBus.ReloadConfig 2>/dev/null >/dev/null \
            && ok "reloaded DBus system config"
        # Restart the shim so it can acquire the names now that the policy allows it
        rc-service cosmic-systemd-shim restart 2>&1 >/dev/null \
            && ok "restarted cosmic-systemd-shim after policy reload"
        sleep 1
    fi

    # Verify the DBus names are now owned
    if command -v dbus-send >/dev/null 2>&1; then
        sleep 1
        for name in org.freedesktop.locale1 org.freedesktop.timedate1; do
            if dbus-send --system --print-reply --dest=org.freedesktop.DBus \
                    / org.freedesktop.DBus.NameHasOwner string:$name 2>/dev/null \
                    | grep -q "boolean true"; then
                ok "shim owns DBus name: $name"
            else
                warn "shim does NOT own DBus name: $name (COSMIC daemons will still get no-such-name)"
            fi
        done
    fi
}
...
${C_BOLD}${C_CYN}== setup-cosmic-alpine.sh complete ==${C_RST}
  ...
  Browser (apk):  ${BROWSER_PKG:-none (use flatpak)}
  Terminal:       wezterm
  CLI utils:      fastfetch, btop, bat, eza, micro, git, wget, curl
  ...
  Audio:          $([ "$DO_AUDIO" = "1" ] && echo "pipewire + wireplumber (autostart)" || echo "disabled")
  Power:          $([ "$DO_POWER" = "1" ] && echo "upower + tuned-ppd (power-profiles)" || echo "disabled")
  Systemd shim:   $([ "$DO_SYSTEMD_SHIM" = "1" ] && echo "locale1 + timedate1 stubs" || echo "disabled")
